...
class DashboardIntegration:
    ...
    async def dashboard_guild(self, user: discord.User, guild: discord.Guild, **kwargs) -> typing.Dict[str, typing.Any]:
        ...
        if not channels:
            return {"status": 0, "error_code": 403, "message": _("I or you don't have permissions to send messages or embeds in any channel in this guild.")}
        ...
        class SendForm(kwargs["Form"]):
            ...
            channel: wtforms.SelectField = wtforms.SelectField(
                _("Channel:"), choices=[], validators=[wtforms.validators.DataRequired()]
            )
            message_id: wtforms.StringField = wtforms.StringField(
                _("Message ID (optional to attach)"), validators=[wtforms.validators.Optional()]
            )
            content: wtforms.TextAreaField = wtforms.TextAreaField(
                _("Message content (if sending new message)"), validators=[wtforms.validators.Optional()]
            )
            emoji: wtforms.StringField = wtforms.StringField(
                _("Emoji"), validators=[wtforms.validators.DataRequired(), wtforms.validators.Length(max=64)]
            )
            role: wtforms.SelectField = wtforms.SelectField(
                _("Role"), choices=[], validators=[wtforms.validators.DataRequired()]
            )
            username: wtforms.HiddenField = wtforms.HiddenField(_("Username:"), validators=[wtforms.validators.Optional()])
            avatar: wtforms.HiddenField = wtforms.HiddenField(_("Avatar URL:"), validators=[wtforms.validators.Optional()])
            data: wtforms.HiddenField = wtforms.HiddenField(
                _("Data"), validators=[wtforms.validators.Optional(), kwargs["DpyObjectConverter"](typing.Any)]
            )
            channels: wtforms.SelectMultipleField = wtforms.SelectMultipleField(_("Channels:"), choices=[], validators=[wtforms.validators.Optional()])
        ...
        return {"status": 0, "web_content": {"source": source, "standalone": True, "send_form": send_form_string}}
